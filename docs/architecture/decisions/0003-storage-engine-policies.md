...
FTS5の削除済みsegment量は`maintenance_state.deleted_since_fts_optimize`で永続化し、削除が10,000行以上蓄積した`DeepIdle`時だけ`optimize`を実行する。optimize後にカウンタをtransaction内でゼロへ戻す。これにより再起動後も、軽いidle maintenanceと重いFTS再編成を区別できる。
