...
final class HistoryFeedModel {
    ...
    ///
    /// `keepingDetail` reports the load without a detail line, so a detail the
    /// caller has just written survives it. Startup recovery is the only caller
    /// that wants this: its outcome would otherwise be replaced by the newest
    /// row's preview before the user has read it. The query is always `.recent`
    /// at that point, so only that path needs to honour it.
    func reload(keepingDetail: Bool = false) {
        switch query {
        case .recent:
